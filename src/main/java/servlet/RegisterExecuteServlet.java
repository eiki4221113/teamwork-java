...
public class RegisterExecuteServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 入力データの取得
		BookAccount account = (BookAccount)session.getAttribute("input_data");
		
		// 登録処理
		int result = BookAccountDAO.registerAccount(account);
		...
		if(result == 1) {
			...
			path = "WEB-INF/view/login-success.jsp";
		} else {
			// 失敗した場合はパラメータ付きで登録画面に戻す
			path = "WEB-INF/view/login-form.jsp?error=1";
		}
